...
#include "../../../wrappers/coarse-set-wrapper.h"
#include "../../genmc/include/genmc.h"

// ["'in(A)-t-0_remove(A)'", "'in(A)-t-0_remove(B)'", "'in(A)-t-0_in(B)-f-0'", "'in(A)-f-0_in(B)-t-0'", "'add(B)_in(A)-f-0'", "'add(B)_in(B)-t-0'", "'remove(B)_remove(A)'", "'in(B)-f-0_remove(A)'", "'in(B)-f-0_remove(B)'"]
set_t set;
