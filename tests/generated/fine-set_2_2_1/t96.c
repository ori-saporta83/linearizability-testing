...
// ["'add(A)_remove(A)'", "'add(A)_remove(B)'", "'add(A)_in(B)-f-0'", "'in(A)-f-0_in(B)-t-0'", "'remove(B)_remove(A)'", "'in(B)-t-1_in(A)-f-0'", "'in(B)-t-1_in(B)-t-0'", "'in(B)-f-0_remove(A)'", "'in(B)-f-0_remove(B)'"]
set_t set;
